...
Tab_PublishSitesToNatSoil <- function() {
  ...
  tabPanel("Publish Sites",  icon = icon("upload", tags$style()),
        ...
        #HTML('<H1 style="font-size:40px;color:darkblue;font-weight:700">Publish Data</H1>'),
             HTML('<BR>'),
            fluidRow( column(width = 9, selectInput('wgtAuthoriser', "Person Authorising Site Data Publication", choices=c('Ross', 'Linda'))),
                      column(width = 3, HTML("<BR>"), shinyjs::disabled(actionButton('wgtPublishSitesBtn', 'Publish Selected Sites', class = "btn-success")))),
            HTML('<BR>
